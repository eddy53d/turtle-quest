...
기존 DB를 쓰는 경우, 기기별 패스키 지원은 `backend/migrate_passkeys.sql`을 한 번 실행하면 적용됩니다.
...
| `POST /api/auth/webauthn/verify-options` → `verify` | `{name}` → 생체 인증 → `{token, user}`. 등록된 기기를 모두 허용 |
